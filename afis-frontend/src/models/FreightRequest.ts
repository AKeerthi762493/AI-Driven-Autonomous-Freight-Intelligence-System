...
export interface IFreightRequest extends Document {
  userId: mongoose.Types.ObjectId; commodityType: string; sourceStation: string;
  destinationStation: string; quantity: number; unit: string; status: FreightStatus;
  assignedRake?: mongoose.Types.ObjectId; assignedRoute?: mongoose.Types.ObjectId;
  estimatedDelivery?: Date; operatorNotes?: string;
  trackingEvents: Array<{ event: string; timestamp: Date; location?: string }>;
...
FreightRequestSchema.index({ userId: 1, status: 1 });
FreightRequestSchema.index({ createdAt: -1 });
